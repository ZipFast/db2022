...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    size_t i = 0;
    for (; i < key.size(); i++) {
      if (!p->get()->HasChild(key[i])) {
        p->get()->InsertChildNode(key[i], std::make_unique<TrieNode>(key[i]));
      }
      p = p->get()->GetChildNode(key[i]);
    }
    if (p->get()->IsEndNode()) {
      latch_.WUnlock();
      return false;
    ...
    auto q = p->release();
    p->reset(new TrieNodeWithValue<T>(q->GetKeyChar(), value));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    latch_.WLock();
    bool res = remove(&root_, root_.get()->GetChildNode(key[0]), key);
    latch_.WUnlock();
    return res;
  }

  bool remove(std::unique_ptr<TrieNode>* root, std::unique_ptr<TrieNode>* child, const std::string& key, size_t depth = 1) {
    if (child == nullptr) {
      return false;
    }
    if (depth == key.size()) {
      ...
      if (!root->get()->HasChildren()) {
        root->get()->RemoveChildNode(child->get()->GetKeyChar());
      }
      return true;
    }
    if (remove(child, child->get()->GetChildNode(key[depth]), key, depth+1)) {
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto node = dynamic_cast<TrieNodeWithValue<T>*>(p->get());
    if (node == nullptr) {
      *success = false;
      latch_.RUnlock();
      return {};
